...
describe("RecommendationsComponent", () => {
      ...
      providers: [
        {
          ...
          useClass: EnhancedRecommendationServiceStub,
        },
  ...
  describe("Component Initialization", () => {
    ...
    it("should have hardcoded sample documents", () => {
      ...
      expect(component.recommendedDocs[0].title).toContain(
        "Efectos de la quema de la caña de azúcar"
      );
      expect(component.recommendedDocs[0].author).toBe(
  ...
  describe("Document Loading", () => {
    ...
    it("should handle empty document response", fakeAsync(() => {
      ...
      expect(component.toastMessage).toBe(
        "Error al recibir los documentos recomendados."
      );
      expect(component.toastType).toBe("error");
    ...
    it("should handle observable error from documents stream", fakeAsync(() => {
      ...
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe(
        "Error al cargar documentos recomendados."
      );
      flush();
    }));
  });
  ...
  describe("Document Categories", () => {
    it("should have documents from different knowledge areas", () => {
      const knowledgeAreas = component.recommendedDocs.map(
        (doc) => doc.knowledge_area
      );
