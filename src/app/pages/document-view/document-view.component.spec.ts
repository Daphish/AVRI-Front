...
describe('DocumentViewComponent', () => {
  ...
  describe('Component Initialization', () => {
    ...
    it('should handle document loading error', () => {
      ...
      expect(component.toastMessage).toBe('Error al cargar el documento. Inténtalo de nuevo.');
    });
  });
  ...
  describe('Repository Viewing', () => {
    it('should open repository URL in new window', () => {
      ...
      expect(window.open).toHaveBeenCalledWith('https://repository.test/doc/1', '_blank');
    });
    ...
    it('should show warning when no repository URI', () => {
      ...
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe('El documento no tiene URL de repositorio disponible.');
    ...
    it('should handle invalid repository URL', () => {
      ...
      expect(window.open).not.toHaveBeenCalled();
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe('No se pudo abrir el enlace del repositorio.');
      expect(component.toastType).toBe('error');
    });
  ...
  describe('Save/Unsave Functionality', () => {
    ...
    it('should save document successfully', () => {
      ...
      spyOn(documentService, 'saveDocument').and.returnValue(of({ success: true }));
    ...
    it('should unsave document successfully', () => {
      ...
      spyOn(documentService, 'removeSaved').and.returnValue(of({ success: true }));
    ...
    it('should handle save error', () => {
      component.saved = false;
      spyOn(documentService, 'saveDocument').and.returnValue(throwError(() => new Error('Save failed')));
      
      component.toggleSave();
      
      expect(component.saved).toBeFalse();
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe('Error al guardar documento. Inténtalo de nuevo.');
      expect(component.toastType).toBe('error');
    });

    it('should handle unsave error', () => {
      component.saved = true;
      spyOn(documentService, 'removeSaved').and.returnValue(throwError(() => new Error('Unsave failed')));
      
      component.toggleSave();
      
      expect(component.saved).toBeTrue();
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe('Error al eliminar de guardados. Inténtalo de nuevo.');
    });
  ...
  describe('Claim/Unclaim Functionality', () => {
    ...
    it('should claim document successfully', () => {
      ...
      spyOn(documentService, 'claimDocument').and.returnValue(of({ success: true }));
    ...
    it('should unclaim document successfully', () => {
      ...
      spyOn(documentService, 'unclaimDocument').and.returnValue(of({ success: true }));
      
      component.toggleClaim();
      
      expect(documentService.unclaimDocument).toHaveBeenCalledWith('claim-doc-1');
      expect(component.claimed).toBeFalse();
      expect(component.showToast).toBeTrue();
    ...
    it('should handle claim error', () => {
      component.claimed = false;
      spyOn(documentService, 'claimDocument').and.returnValue(throwError(() => new Error('Claim failed')));
      
      component.toggleClaim();
      
      expect(component.claimed).toBeFalse();
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe('Error al reclamar documento. Inténtalo de nuevo.');
    });

    it('should handle unclaim error', () => {
      component.claimed = true;
      spyOn(documentService, 'unclaimDocument').and.returnValue(throwError(() => new Error('Unclaim failed')));
      
      component.toggleClaim();
      
      expect(component.claimed).toBeTrue();
      expect(component.showToast).toBeTrue();
      expect(component.toastMessage).toBe('Error al liberar documento. Inténtalo de nuevo.');
    });
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should handle error recovery scenarios', () => {
      ...
      spyOn(documentService, 'saveDocument').and.returnValue(throwError(() => new Error('Network error')));
      component.toggleSave();
      ...
      (documentService.saveDocument as jasmine.Spy).and.returnValue(of({ success: true }));
      component.toggleSave();
  ...
  describe('Edge Cases', () => {
    it('should handle document with special characters in URI', () => {
      const mockDocument: DocumentDetail = {
        ...
        knowledge_area: 'Information Science',
        license: 'MIT',
        repository_uri: 'https://test.com/docs/título%20español?query=test&lang=es#section1',
      ...
      expect(window.open).toHaveBeenCalledWith(mockDocument.repository_uri, '_blank');
    });
