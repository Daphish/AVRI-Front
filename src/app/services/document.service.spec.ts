...
describe("DocumentService", () => {
  ...
  it("getDocumentsByIds() preserves order and does NOT dedupe internally", (done) => {
    ...
    const reqs = http.match(
      (req) =>
        req.url.includes("/api/documents/") && req.url.includes("/repository")
    );
  ...
  describe("Integration Tests", () => {
    ...
    it("should handle save -> remove workflow", () => {
      ...
      const removeReq = http.expectOne(
        "/api/documents/saved/delete/workflow-doc/"
      );
    ...
    it("should handle claim -> unclaim workflow", () => {
      ...
      service.claimDocument(docId).subscribe();
      const claimReq = http.expectOne(
        "/api/documents/authored/add/authored-doc/"
      );
      claimReq.flush({ id: "authored-1", document: mockDocument });

      service.unclaimDocument(docId).subscribe();
      const unclaimReq = http.expectOne(
        "/api/documents/authored/delete/authored-doc/"
      );
