...
describe('ChatService — extra coverage', () => {
  ...
  beforeEach(() => {
    ...
    authSpy = jasmine.createSpyObj<AuthService>('AuthService', ['markProfileAsCompleted']);
  ...
  it('createSession() without name uses default "Chat sin título" and updates state', (done) => {
    ...
    post.flush({ session_id: 's-new-default', session_name: 'Chat sin título' });
  });
        ...
        messages: [
          {
            role: 'assistant',
            content: 'Hello ##12$$  ',
            reference: { chunks: [{ document_id: 'd1' }, { document_id: 'd2' }, { document_id: 'd1' }] },
          },
        ],
      }],
    });
      ...
      messages: [
        {
          role: 'assistant',
          answer: 'Answer ##99$$',
          reference: [{ document_id: 'x1' }, { document_id: 'x1' }, { document_id: 'x2' }],
        },
      ],
    });
  ...
  it('sendMessage(): if loading message is missing, pushes reply (covers else path)', (done) => {
    ...
    post.flush({
      data: {
        answer: 'Ok!',
        reference: { chunks: [{ document_id: 'a' }, { document_id: 'b' }, { document_id: 'a' }] },
      },
    });
