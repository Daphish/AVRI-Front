...
describe("AuthService (aligned with project code)", () => {
  ...
  const matchCreateAnon = (u: string) =>
    /\/api\/(user\/)?create-anonymous\/?$/.test(u);
  const matchTokenAnon = (u: string) =>
    /\/api\/(user\/)?token-anonymous\/?$/.test(u);
  const matchProfileMe = (u: string) =>
    /\/api\/recommender\/profile\/me\/?$/.test(u);
